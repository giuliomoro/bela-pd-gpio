/*
 * Adding BELA_LIBPD_GPIO capabilities to Bela's core/default_libpd_render.cpp@87ddf18a4001dc32bdc8e64451a0144ee759da2b
 */
#include <Bela.h>
...
#ifdef BELA_LIBPD_DISABLE_GPIO
#undef BELA_LIBPD_GPIO
#endif // BELA_LIBPD_DISABLE_GPIO
...
static std::vector<LibpdGpio> gGpios;
static void sendGpioToPd(LibpdGpio* g)
{
	if(Gpio::INPUT != g->direction)
		return;
	bool current = g->gpio->read();
	bool shouldSend = g->shouldSend;
	if(!shouldSend)
	{
		if(g->lastValue != current)
			shouldSend = true;
	}
	if(!shouldSend)
		return;
	libpd_start_message(2);
	libpd_add_float(g->pin);
	libpd_add_float(current);
	libpd_finish_list("bela_gpioIn");
	g->lastValue = current;
	g->shouldSend = false;
}

static LibpdGpio* findGpio(const unsigned int pin)
{
	// TODO: sort gGpios by pin when added and do a binary search here
	for(auto& g : gGpios)
	{
		if(pin == g.pin)
			return &g;
...
#endif // BELA_LIBPD_GUI
#ifdef BELA_LIBPD_GPIO
	if(0 == strcmp(source, "bela_gpioOut"))
	{
		if(argc < 2 || !libpd_is_float(argv) || !libpd_is_float(argv + 1))
			return;
		const unsigned int pin = libpd_get_float(argv);
		const bool value = libpd_get_float(argv + 1);
		LibpdGpio* g = findGpio(pin);
		if(g)
		{
			if(Gpio::OUTPUT == g->direction)
				g->gpio->write(value);
		}
		return;
	}
...
#endif // BELA_LIBPD_TRILL
#ifdef BELA_LIBPD_GPIO
	if(strcmp(source, "bela_setGpio") == 0){
		// symbol is the direction (in, out, disable), argv[0] is the
		// pin, argv[1] (optional) is initial output (0 by default)
		if(argc < 1)
			return;
		Gpio::Direction direction;
		if(libpd_is_float(argv) == false)
			return;
		unsigned int pin = libpd_get_float(&argv[0]);
		if(strcmp(symbol, "in") == 0){
			direction = Gpio::INPUT;
		} else if(strcmp(symbol, "out") == 0){
			direction = Gpio::OUTPUT;
		} else if(strcmp(symbol, "disable") == 0){
			// go backwards to avoid size() and indexes changing after removals 
			for(ssize_t n = gGpios.size(); n >= 0; --n)
			{
				if(pin == gGpios[n].pin)
				{
					delete gGpios[n].gpio;
					gGpios.erase(gGpios.begin() + n);
				}
			}
			return;
		} else {
		...
		bool initialValue = 0;
		if(Gpio::OUTPUT == direction && argc >= 2 && libpd_is_float(argv + 1))
			initialValue = libpd_get_float(argv + 1);
		rt_printf("pin: %d, direction: %d, val: %d\n",
			pin,
			direction, initialValue);
		LibpdGpio* dest = findGpio(pin);
		// if we already have a Gpio on this pin, we ovewrite it
		if(!dest)
		{
		...
		dest->gpio->open(pin, direction);
		dest->shouldSend = false;
		if(Gpio::OUTPUT == direction)
			dest->gpio->write(initialValue);
		else {
			// the initial read is sent to Pd during the next audio callback.
			// Can't do it now in case we are being called during
			// libpd_openfile() because of
			// https://github.com/libpd/libpd/issues/274 (???)
			dest->shouldSend = true;
		}
		return;
	}
#endif // BELA_LIBPD_GPIO
...
#ifdef BELA_LIBPD_GPIO
	if(strcmp(source, "bela_gpioRead") == 0)
	{
		LibpdGpio* g = findGpio(value);
		g->shouldSend = true;
		return;
	}
#endif // BELA_LIBPD_GPIO
...
#endif // BELA_LIBPD_TRILL
#ifdef BELA_LIBPD_GPIO
	libpd_bind("bela_setGpio");
	libpd_bind("bela_gpioRead");
	libpd_bind("bela_gpioOut");
#endif // BELA_LIBPD_GPIO
